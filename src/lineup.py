...
def calculate_lineup_liquido(
    lineup_bruto: int, cancelados: int, adiados_14d: int
) -> int:
    return max(0, lineup_bruto - cancelados - adiados_14d)


def calculate_taxa_cancelamento(
    cancelados_7d: int, adiados_14d_7d: int, lineup_bruto_7d_atras: int
) -> float:
...
def calculate_var_semanal(
    lineup_liquido_atual: int, lineup_liquido_7d_atras: int | None
) -> float | None:
    if lineup_liquido_7d_atras is None or lineup_liquido_7d_atras <= 0:
        return None
    var = (
        (lineup_liquido_atual - lineup_liquido_7d_atras) / lineup_liquido_7d_atras * 100
    )
    return round(var, 2)
...
def validate_lineup(lineup_bruto: int, lineup_liquido: int) -> list[str]:
    ...
    if lineup_liquido > lineup_bruto:
        errors.append(
            f"lineup_liquido ({lineup_liquido}) > lineup_bruto ({lineup_bruto})"
        )
    if lineup_bruto < 0:
    ...
    if lineup_bruto_7d_atras is not None:
        ...
        if not (0 <= taxa_cancelamento <= 100):
            validation_errors.append(
                f"taxa_cancelamento fora de range: {taxa_cancelamento}"
            )
